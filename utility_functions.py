...
# Pipe-table validation & rendering
...
def _looks_like_pipe_table(s: str) -> bool:
	"""
	Detects markdown pipe tables or escaped-newline pipe tables inside a cell.
	Examples:
	- "| A | B |"
	- "\\n| --- | --- |"
	"""
	if not s:
		return False
	t = s.strip()
	# fast checks for typical table patterns
	if "\\n|" in t:
		return True
	if t.startswith("|") and "|" in t[1:]:
		return True
	# common case: "A. | Col | ..." (table after a label)
	if "| ---" in t or "|\n| ---" in t:
		return True
	return False
